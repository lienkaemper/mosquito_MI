...
from src.gen_cov_mats import sparse_latent_cov
from src.clustering import order_cor_matrix
from src.gen_cov_mats import cov_to_cor



parser = argparse.ArgumentParser(description='Find exprssion matrixes which maximize mutual information with Carey-Carlson used for sensing matrix')
...
MI_curves = np.zeros((trials, n_grid_e, n_grid_n, opt_steps))
with tqdm(total = n_grid_e * n_grid_n * trials) as pbar:
    for trial in range(trials):
        #Cov_xx= rand_dot_mat(n_o, 10)
        p_source = .15
        p_sample = 1
        n_source_signal = 25
        n_source_noise = 1000
        ...
        for i, var_e in enumerate(var_es):
            for j, var_n in enumerate(var_ns):
                if max(var_e, var_n) > 0:
                    x, gradients, values ,xs =maximize_mi(receptor_cov_t, noise_cov_t, env_noise = var_e, neur_noise = var_n, m = n_n, steps = opt_steps, rate = .05, grad_noise = .05)
                    A = x.detach().numpy()
...
with open('../results/tables/opt_coexp_cc/stim_covariance.pkl', 'wb') as file:
    pkl.dump(Cov_xx, file)
...
with open("../results/tables/opt_coexp_cc/MI_curves.pkl", 'wb') as f:
