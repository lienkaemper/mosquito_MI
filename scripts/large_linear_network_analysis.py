    ...
    for i, var_e in enumerate(var_es):
        for j, var_n in enumerate(var_ns):
            ...
            for trial in range(trials):
                ...
                # compute coexpression level 
                coexp_idx.append(coexp_level(A))
                
                # compute response gain and PCA alignment 
    ...
    for i, var_e in enumerate(var_es):
        for j, var_n in enumerate(var_ns):
            ...
            for trial in range(trials):
                ...
                #determine co-expressed receptors 
                C, N = coexd_receptors(A, thresh = .1)

                #compute co-expression pressure for coexpressed, non-co-expressed receptors
